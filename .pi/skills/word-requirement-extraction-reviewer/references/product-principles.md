...
Witness 固定提交 `exclude` 与 `select` 两个必填、非 nullable array lane；`exclude` 可提交 0-3 张彼此独立的反例 card，`select` 可提交 0-1 张，空 lane 精确为 `[]`，不使用 `kind=none` sentinel。每张 card 只有一个连续 target range、一句 premise 和 1-8 个 supporting focus block；direction 由外层 lane 机械派生，不是模型字段。lane 名、card 数量、card 顺序和 supporting block 数量都没有证据权重，不能形成投票。Witness 必须先跨 source-proven functional partition 覆盖反例，所有其他可见 partition 扫描完成且没有更强独立反例后，才可把第二张卡用于同一连续 premise cluster；这个排序完全由模型完成，Harness 不识别 partition 或 cluster。该容量只允许一次 Witness 同时暴露少量独立反例，不改变 source quote 总量/单 block 上限、三次 provider call 上限或 Harness 语义边界，也不得扩张为 mini-ledger。全局 JSON/turn/schema/cross-field 与完整 source 地址合同必须先通过，随后代码才逐 card 对 target 的 focus、provisional state、同一连续 group、`AUDIT_UNIVERSE` 和 support 的全局 focus 权限做机械校验。source 中真实存在但未获动态权限的 card 必须整卡弃权，不能裁剪或修补；同 lane 其他有效 card 与另一 lane 独立保留。空 lane 为 `valid_none`，至少一张有效 card 为 `valid_challenge`，非空且全部 card rejected 才为 `rejected_source_focus`；任一卡 rejected 形成 `partial` coverage，只有双 lane 都完全 rejected 才阻断发布。exclude card 必须由目标 block 自身的肯定 exclusion predicate 支撑，并在提交前主动反证 target 内不存在 surviving action/state/result/standard-applicability predicate；普通重复、语义冗余、可由别处覆盖或结果更短/整洁都不能单独授权排除。Finalizer 第二轮必须逐 card 独立核验有效 predicate；rejected card 的 lane/index、range、premise、supporting ID、typed reason 与 error 只能留在 trace 且 `forwarded=false`，Finalizer 只能看到中性 coverage/lane status、card index 和有效 challenge。

V32 将上述 partition 覆盖收敛为一个 global card tournament：先枚举全部可见候选，再执行 eligibility 与 whole-block survivor veto；任一独立 action/state/result/standard-applicability/heading survivor 都使 exclude 候选彻底失去资格，而不是仅降权。剩余 eligible candidates 按肯定 exclusion mechanism × source-proven peer-bounded partition 排序；同一 selected island 且同一 mechanism/family 的第一轮最多一张，同一连续 remedy/price/proof cluster 不得拆卡制造票数。只有其他 eligible family/partition 均为空时才可回到同 family。该排序只由 Witness 完成，Harness 不识别 predicate、partition、family 或 cluster。
...
Active Pi-native 的最终集合还必须执行 `symmetric_partition_fixed_point` 与 `semantic_projection_checksum`：每个 selected compact range 都是其内部每个 canonical block 均有 membership 的全称命题，必须寻找 target-own 肯定 exclusion 反例并拆出 holes；每个 excluded gap 都是其内部每个 block 均无 membership 的全称命题，必须寻找 target-own survivor 或真实 heading/body/table closure 反例并恢复。宽 range、组摘要、端点、相邻正例和同 subsection 的其他 survivor 都不能替内部 block 证明。双向攻击结束后先冻结最终 block-ID set `S`，再由 `S` 唯一生成 typed ranges；任何已明确判定的 hole 都不得出现在展开后的 typed selection，任何已明确判定的 survivor 都必须出现，内部 hole 必须拆开 range。reason 不得再写一份最终 compact ranges 与 typed fields 竞争。该过程只在模型内部完成，不增加 schema、调用、ledger 或代码语义判断。

V32 的 fixed point 是 targeted delta 的双向验证，不是重生一份整文答案。第二轮先冻结 provisional block-ID set `S0`；`S0` 不是真值，但是唯一集合编辑基线。先关闭 reason/typed set 自相矛盾，再不依赖 Witness 卡对全部 selected blocks 执行 exclusion closure、对全部 provisional excluded gaps 执行 survivor closure，然后核验 Witness，最后做 delta projection。每个变化必须由 exact target-own predicate 成立并进入 `Δ-` 或 `Δ+`，终态只能是 `S=(S0-Δ-)∪Δ+`；不得从新 reason、章节摘要或整段分组重新生成 selection。`Δ-` 不得出现在 final selection，`Δ+` 必须全部出现。该过程只在模型内部完成，不增加 schema、调用、ledger 或代码语义判断。

Active Pi-native 还必须遵守 canonical block / table-block atomicity：不可分单位只能是恰好一个 canonical block object / 一个 block ID，不能扩张到 subsection、sibling group 或 compact range；其他 block 的 survivor 不能借给目标 block。同一个 table block 内的 row/cell 不是独立输出地址。在四类 carrier 外，只要任一 row/片段仍有合格对象、参数、人员、工期、质量、服务或结果事实，就不得因同表另有付款、价格、合同指针等 row 删除整个 block；只有整块剥离后 surviving remainder 为零才可排除。Witness 在声称目标“仅为 X”前必须覆盖该 block 内全部独立命题，任一 action/state/result/standard-applicability survivor 都阻断整块删除。Witness 不得提交 row-level 删除 premise，Finalizer 不得把 row-level rationale 投影成整块 exclusion；模型内部可逐 proposition / row 阅读，但不增加 schema、patch 地址或 ledger。该 atomicity 不会覆盖 hard-carrier Owner gate。
