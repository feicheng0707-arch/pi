...
Candidate-S0 v33 Challenger 的 JSON 顶层恰好只有 `remove_partitions`、`remove_audit_partitions` 与 `add_partitions` 三个 non-nullable arrays，不传输任何 hard-root claim、root span 或 root navigation group。该路线的 exact remove/add partition 是 singleton exact card：`target_ranges` 恰有一个 `段落N`，一个 card 只表达一个 canonical block；canonical table block 使用其唯一 block 地址。Remove 最多八张、add 最多两张，禁止多目标 partition、连续 range、block 内切片或 Harness 拆分 salvage。Exact supporting transport/schema hard cap 是四个且必须包含 target；audit transport cap 仍是三十二个、协议 cap 仍是十二个。超过 exact transport/schema cap 会使完整 Challenger response fail-closed；audit 十三至三十二个 supporting IDs 在全局 schema 通过后逐 partition 完整拒绝。两类都绝不截断、抽样或保留前 N 个。Challenger 必须先完整枚举 source-certain remove/add targets；只按同一方向、同一 block 地址消除完全重复项。Remove 候选不超过八张、add 候选不超过两张时必须逐目标全部提交；只有超过对应上限时才按 source 确定性、材料性与独立失败覆盖排序取舍。相邻、连续、`source_conclusion` 相同、机制相同或同属一个 cluster 都不能在容量未满时合并或省略。内部逐 block closure 不进入输出，未挑战 block 不生成 verdict，因此该规则仍是 bounded sparse review，不是 ledger。当前 active 三调用 Witness 的四卡 tournament 及“同一 root/source conclusion/连续 cluster 最多一张”只约束该 active 路线，不适用于 Candidate-S0 v33；两条路线不得互相继承 card grouping 或 slot 压缩规则。Challenger 仍必须在内部先完成 recovery/root 判定：先冻结肯定成立的 recovery boundary，再建立 source-proven hard-root spans，并让全部 hard-root descendants 对三个 ordinary arrays 保持沉默；内部 root map 不进入 JSON、trace semantic claim 或 Finalizer 权限。Finalizer 不依赖 Challenger root 提示，必须从完整 source 独立完成 global hard-root sweep，并仍可通过最多三十二个 typed veto 发布 source-proven root projection。即使全部非 recovery ordinary partitions 均被拒绝，非空 `S0` 仍必须进入独立全 `S0` Finalizer并记录 `coverage=none`；被拒绝地址不获得 keep 票或 add envelope。任何 submitted `recovery_boundary_scope` 被拒绝仍使整次 run fail-closed。Challenger 继续只允许一次 `response_format={"type":"json_object"}` JSON text，完整成功路径仍最多两次 provider call，不增加 retry、角色或 ledger。Finalizer 仍必须令 canonical ordinary remove 与有效 root projection 零重叠；若唯一提交中出现同向交集，Harness 只能把该交集机械归一化为 root-owned removal并记录冗余 trace，因为它不改变最终集合或扩大删除权限。无效 root 不得遮蔽 ordinary remove，完整 `S0` ordinary echo、add/root 冲突和 root/root 重叠仍必须 fail-closed；代码不得借此判断任何 root 或删除在语义上正确。

Candidate-S0 两调用实验允许同一个 source-certain sparse hole 同时出现在 singleton exact remove card 与一个完整 typed audit scope 中。该双通道只解决 review navigation 的信息损失：每张 singleton exact card 独立定位一个已肯定原子 hole，audit 仍保留完整 recovery/mixed boundary 并要求 Finalizer 逐 block 闭包。同一 audit 内存在多个已肯定 hole 时，每个 hole 仍独立进入 exact 候选集合；容量未超八张时不得因 audit overlap、地址相邻或 premise 相同而省略，超限后才统一排序。两者不是两票、双重证据或额外删除授权，exact-overlap blocks 的并集必须是每个完整 audit target set 的严格子集；不得裁剪 audit、用 supporting IDs 代替已肯定 exact hole，或用 exact channel 预删整个 audit。Harness 只可按地址集合机械校验 exact-overlap strict subset、audit-audit 去重、block 预算与最终 wholesale-removal 禁令，不得判断何者在语义上是 hole。任何 submitted recovery audit 被机械拒绝仍须整次 fail-closed。
