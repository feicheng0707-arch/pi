...
- 空 Candidate 的 `pass` / 机械 no-op 在一调用后结束；非空 Candidate 的 `pass` / 机械 no-op 必须进入一次独立、受保护的 terminal-or-hard Release audit；
...
- `reviewer_no_change_terminal_or_hard_veto` 模式是独立的保守对抗审查，不是第二次完整提取：全部 Candidate 均为 `BASE_KEEP`，`REMOVE_REVIEW`、`ADD_REVIEW`、`BOUNDARY_REVIEW`、普通 OUT 新增以及 typed hard delta 之前的载体外删除全部禁用。该模式既用于非空 Candidate 的 Reviewer pass / 机械 no-op，也用于 Reviewer schema、range 或 terminal contract 无效后的安全降权：无效 Reviewer 提案整体丢弃，Harness 不从 reason、正文、Candidate 或默认业务假设补猜任何字段。若完整 source 独立证明整文是非采购文档、供应商已完成响应、纯合同或未实例化模板，Release 必须把恰好完整 Candidate 写入 `outside_carrier_excluded_ranges`，并保持其他 delta 为空；否则先只能把独立证明属于四类硬排除载体的 Candidate descendants 写入 `hard_excluded_ranges`。该 hard delta 至少命中一个 Candidate block 后，Harness 才可把 `Candidate - hard exclusions` 的完整地址 residual 暴露给同一次 Release；一个连续 residual 沿用原有权限，离散 residual 只有在不超过 3 个地址岛、64 个 Candidate blocks 且不超过 Candidate blocks 的 10% 时才获得 outside-carrier precision。Release 仍须按 `pre_award_stage_gate`、`outside_carrier_precision_closure`、`pricing_basis_role_attack`、`non_fact_shell_closure` 和 duty 反例逐语义证明每个提交删除的 atom；代码不读取正文。若 terminal veto、hard carrier 和授权 residual 删除均不成立，必须提交空 delta并原样保留 Candidate。Harness 只以地址集合机械授权完整 Candidate terminal veto 与 residual envelope，越界提交会被裁剪；
- 若 Reviewer 同时提交 `source_role=buyer_issued`、`instantiation=present`，却以普通 removal 覆盖全部 Candidate，Harness 将其视为灾难性误删形状而不是语义结论：在 Release 前撤销完整普通 remove envelope，使全部 Candidate 初始为 `BASE_KEEP`，保留 `ADD_REVIEW` 与 Candidate 全域四类 hard-carrier sweep，并禁用 `BOUNDARY_REVIEW`。Release 提交 typed hard delta 后，至少一个 Candidate block 必须进入 `hard_excluded_ranges`；随后 Harness 只按地址计算完整 `Candidate - hard exclusions` residual。一个非空连续地址岛沿用原有 precision 权限；离散 residual 仅在不超过 3 个地址岛、64 个 Candidate blocks 且不超过 Candidate blocks 的 10% 时授权。结果为空、岛数/blocks/比例任一越界或没有 Candidate hard exclusion都不授权。代码不读取正文、不决定 residual 中任何 block 的语义，Release 仍须把每个准备删除的 atom 明确写入 `outside_carrier_excluded_ranges`；
- 普通 bounded patch 也可获得同一 bounded residual 权限，但解锁条件更严格：Release 必须先独立把至少一个属于 Reviewer `REMOVE_REVIEW` envelope 的 Candidate block 写入 `hard_excluded_ranges`；仅在 `BASE_KEEP` 中发现 hard carrier 不能解锁。随后仍只按完整 Candidate 地址集合减去全部 hard exclusions；一个非空连续 residual 沿用原有权限，离散 residual 只有在不超过 3 个地址岛、64 个 Candidate blocks 且不超过 Candidate blocks 的 10% 时授权。该权限只补足“Reviewer 找到大 hard carrier 后对少量存活 residual 早停”的共模风险，不是第二次完整提取；没有 confirmed `REMOVE_REVIEW` hard hit、没有剩余 Candidate 或 residual 任一机械门越界都不授权；
- Release 必须先写 `hard_carrier_reason`，完成整文与每个连续 Candidate interval 的四类载体 Owner/peer-root 假设；Candidate interval 的首个 block 可能只是一个起于 OUT 的四类 root descendant，必须向前检查实际 ancestor root。再写 `residual_reason`，在任何 range 字段之前对该假设执行反向边界攻击，纠正向前吞并合格章节、越过不同 Owner peer exit 或遗漏后续 qualified peer root 的判断，并关闭载体外授权 atom。两个 reason 都完成后才投影三个 typed delta：`hard_excluded_ranges` 必须精确投影 `residual_reason` 最终仍确认的每个四类 root→semantic-exit 与授权 Candidate/`ADD_REVIEW`/`BOUNDARY_REVIEW` 的完整交集，不得只写其中价格、程序或 Reviewer 已挑战的零散子集。若要保留 root 内任一 descendant，必须先在 `residual_reason` 中撤销、缩窄该 root 或证明更早的不同 Owner peer exit；已经确认的 root 内禁止再用技术价值、主要直接效力或 `duty_survival_attack` 挖洞。`outside_carrier_excluded_ranges` 通常只能投影 `REMOVE_REVIEW` / `ADD_REVIEW` / `BOUNDARY_REVIEW` block；满足当前 hard-boundary residual 解锁条件时，还可投影由同次 hard delta 机械派生且满足 bounded minority 门的完整 Candidate residual，表达载体外经 `duty_survival_attack` 后可安全分离的 non-requirement atom。`accepted_add_ranges` 只能批准独立验证的 `ADD_REVIEW`，或满足完整 shell-body 闭合合同的 `BOUNDARY_REVIEW`。Harness 只删除两个 exclusion 字段明确授权的 block，并把其余 `REMOVE_REVIEW` 机械恢复。Harness 固定派生 `final = Candidate - authorized hard exclusions - authorized outside-carrier exclusions + accepted challenged additions`。若 accepted add 与 exclusion 重叠，accepted add 优先，冲突 exclusion trace 被机械裁剪。Release 不提交完整 `final_ranges`，Harness 不解释两个 reason、不识别载体、不补写语义；
...
- `survivorProjectionInvariant`：Release 的 `residual_reason` 必须在最终字段前用一个紧凑 `survivor_projection` 汇总本轮已经判定必须存活的授权地址岛。凡被 Phase 2 认定为直接工作义务、必要 source-fidelity 正文或其他安全保留内容的 block，都不得出现在 `hard_excluded_ranges` 或 `outside_carrier_excluded_ranges`；若草稿字段仍有交集，必须先拆分、撤销错误 hard 假设或修正字段，再提交唯一工具调用。反过来，`survivor_projection` 不能在已经最终确认的四类 root 内挖洞；要保护该 descendant，必须先在 reason 中用 source-proven 边界撤销或缩窄 hard root。该不变量只约束同一次 Release 的 reason→typed delta 投影，不新增字段、逐 block ledger、模型调用或代码语义判断；
- `boundedResidualPartitionInvariant`：同次 typed hard projection 若按当前模式解锁 `Candidate - hard exclusions` residual，Release 必须在 `residual_reason` 的前部用一个紧凑 `bounded_residual_partition=scope:<ranges>;outside:<ranges or none>;survive:<ranges or none>` 完整覆盖每个 residual 地址。`outside` 必须逐岛通过载体外原子门并投影到 `outside_carrier_excluded_ranges`，`survive` 必须进入 `survivor_projection` 并从两个 exclusion 省略；工程量清单编制/计价说明、裸图纸或附件指针、空壳与独立技术规范等不同 peer 不能因同属 residual 而整体跳过或整体继承。若 residual 未解锁则写 none，不得伪造权限。该三分区是一次 compact range closure，不是逐 block ledger，不增加字段、调用或代码语义裁决；
- `boundaryReviewDirectionContract`：`BOUNDARY_REVIEW` 只允许在 Candidate 已选 shell 之后补齐遗漏的必要正文、表格、清单、续行或附件，不允许向前补可选标题、项目名、文件元数据或导航标签。Candidate 已经含有实质 body 时，前置标识不因地址相邻而获得 membership；真正不可替代的前置标题必须由 Reviewer 通过 `ADD_REVIEW` 明确提出，再由 Release 独立裁决；
- `contractAttachmentSequenceContract`：实际合同 root 内连续编号的附件、责任书、清单和表格继续继承合同 Owner。技术标题、参数密度、附件编号重启或局部技术价值都不能单独形成 exit；只有 source 证明的同级或更高层级、功能不同的 peer root 才能结束合同序列；
...
- Reviewer provider/capacity/timeout/budget failure、空 Candidate 的 contract failure、Release failure、总预算或越界都静态保留 candidate 并标记 degraded。非空 Candidate 的 Reviewer schema/range/terminal contract failure 不得获得任何 Reviewer patch 权限，但可继续执行已预算的 `reviewer_no_change_terminal_or_hard_veto`；只有该独立 Release 明确提交且经地址 envelope 授权的 terminal、hard 或 bounded residual typed delta 可以改变 Candidate，Release 再失败则原样保留。
...
纯封面项目名、文件名、分册/卷册名、图片或版式占位、目录标签和其他导航性标识，不会仅因准确指向当前真实项目而获得 requirement membership。只有当该标题是理解同一合格 Owner 下、在 peer exit 前实际存活正文不可替代的结构上下文时才保留；若剥离该标识后没有现存对象范围、动作、参数、义务、结果或必要正文，它就是 `cover_navigation_shell`，应作为精确 outside-carrier atom 排除。除非 source 另行证明它实际位于四类 root 内，否则不得为了越过 `BASE_KEEP` 把导航壳伪装成 `hard_excluded_ranges`。该规则不能向后删除相邻合格正文，也不能否定由 source 其他已填写事实建立的项目实例化。
...
Release 不提交完整最终集合，只提交 `hard_excluded_ranges`、`outside_carrier_excluded_ranges` 与 `accepted_add_ranges`。Harness 将 hard exclusion 裁剪到 Candidate、`ADD_REVIEW` 或 `BOUNDARY_REVIEW`，将 outside-carrier exclusion 裁剪到 `REMOVE_REVIEW ∪ ADD_REVIEW ∪ BOUNDARY_REVIEW`，并在当前 hard-boundary residual 模式满足解锁条件时额外允许落入由同次 hard delta 机械派生且通过连续或 bounded minority 门的完整 Candidate residual；accepted add 只裁剪到 `ADD_REVIEW ∪ BOUNDARY_REVIEW`。随后只减去两个 exclusion 字段明确授权的 block，把其余 `REMOVE_REVIEW` 机械恢复并派生 `restoredRemoveRanges` trace。accepted add 对重叠 exclusion 优先；冲突 exclusion trace 随之去除最终保留地址并消除重叠，再按固定公式派生 final。Harness 只执行这些集合约束，不读取 reason，不推断 Owner、membership 或安全删除语义。
...
- 对非空 Candidate 的 Reviewer contract failure 执行权限降级：完整 Reviewer proposal 作废，不生成 challenge overlay，只进入与 pass/no-op 相同的受保护 terminal-or-hard Release audit；该降级只依据 contract validity 和 Candidate 是否为空，不读取正文、reason 或 case 身份；Release 只有先提交 typed hard delta 后，才可能获得同次地址派生的 bounded residual 权限；
...
- 仅按 Candidate/remove/hard 地址集合检测完整 removal、撤销普通 envelope，并机械派生 hard-boundary residual 权限：full-removal safety 与 Reviewer no-change hard audit 接受任一 Candidate hard hit；普通 bounded patch 必须至少有一个 hard hit 同时属于 `REMOVE_REVIEW`。随后只计算 `Candidate - hard exclusions` 的地址岛、block 数与比例：一个非空连续 residual 直接授权；离散 residual 只有在不超过 3 个地址岛、64 个 Candidate blocks 且不超过 Candidate blocks 的 10% 时授权；
...
`operational_precision` 的 10% Candidate 字符门是机械安全门，不是代码语义判断。只有模型已经把本次 challenge 声明为 `operational_precision`、且机械裁剪后的有效 patch 没有新增 block 时，Harness 才按完整 Candidate 与拟删地址的字符数计算比例；低于门槛就机械归一化为 `reviewer_noop_challenge`。若同一提交存在有效新增，`operational_precision` 与其 remove-only schema 机械矛盾，Harness 只把非权威 issue label 降为 `unspecified` 后交给独立 Release，不读取 reason 或正文、不决定新增是否正确。空 Candidate 保留后结束；非空 Candidate 的低于门槛 remove-only challenge 不获得普通 patch 权限，只进入受保护 terminal-or-hard Release audit。代码不得根据正文内容重新分类 issue type，也不得把该比例用于任何 correctness challenge。
...
- 非空 Candidate：固定最多新增 1 次 Doubao 2.0 Pro + 1 次 GLM 5.2 Release 调用，Release reasoning 关闭；contract-valid challenge 走 bounded patch Release，pass / 机械 no-op / Reviewer contract failure 走受保护 terminal-or-hard Release audit；两者都不增加第三次调用，bounded residual 只复用同一次 Release；
- 无 retry、第三角色、投票、best-of-N、逐 block ledger 或自由读搜 loop；
- Reviewer provider、capacity、timeout、budget failure，空 Candidate contract failure，或任一 Release failure：0 次语义改写，candidate 原样保留并标记 degraded。非空 Candidate 的 Reviewer schema/range/terminal-contract failure 只撤销 Reviewer 权限，不阻断同一固定两调用预算内的独立 terminal-or-hard audit；不得把它实现成 Reviewer retry、答案驱动路由或普通 patch fallback。
- schema 合法但经 candidate membership 机械归一化后净变化为空的 challenge，不是语义失败；空 Candidate 记录 `reviewer_noop_challenge` 后结束，非空 Candidate 进入受保护 terminal-or-hard Release audit。
- 模型声明为 `operational_precision` 且拟删字符低于完整 Candidate 10% 的 challenge，同样机械归一化为 no-op；空 Candidate 结束，非空 Candidate 只进入受保护 terminal-or-hard Release audit。该门不适用于模型声明的 correctness issue。
