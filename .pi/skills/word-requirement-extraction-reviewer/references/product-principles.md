...
运行时按角色切片加载 Prompt：Finalizer 加载 mode-specific 的 `pi-native-semantic-contract.md`、`pi-native-runtime-contract.md` 与 `finalizer.md`；Witness 只加载自包含的 `witness.md`，再接收当前 run 的有界 focus、结构化 provisional ranges/claims、exact JSON schema，以及 schema-bounded `UNTRUSTED_PROVISIONAL_RATIONALE`。Rationale 的唯一用途是让 Witness 看见作者已经实际采用的 claim 并对其做可证伪攻击；它不增加 source evidence、地址权限、card 权重或语义权威。Witness 必须以 source 与实际地址决定结论，不得引用 rationale 作为 premise support，也不得因 rationale 未提及某地址就跳过全局扫描。不得把 Finalizer 的发布、neutral replay、trace 与 Harness 预算规则重复拼进 Witness system prompt。本文仍是治理和评估真源，但不整篇塞入模型上下文。这样避免旧模式字段、其他角色、重复语义规则和评估说明干扰窄职责判断。
