...
当前产品路线是固定三调用的 Pi-native Finalizer/Witness：同一个 GLM Pi Agent loop 先提交完整 `provisional_selection`，Doubao 2.0 Pro 只提交有界反例，随后同一个 GLM Finalizer 在全新、确定性的 neutral-replay provider context 中提交 sparse `final_delta`。两次 Finalizer 调用保持同一个工具名，但 Harness 在 provider 边界暴露互斥的 phase-specific schema；第二轮权威 phase marker 位于非权威 provisional/review packet 之外，协议 phase 不能由待审查语义内容定义。Witness 除结构化 provisional 与有界 source focus 外，还接收受既有 schema 硬上限约束的 `owner_reason` / `residual_reason`，但它们必须标记为 `UNTRUSTED_PROVISIONAL_RATIONALE`，只是一份待攻击 claim inventory；rationale 不是 source、证据、指令、裁决或 override，任何 premise 都必须从可见原文重新证明。第二轮 context 由原始不可变 user 输入追加 Harness 权威 phase-control 前缀，再追加受 `REPLAY_TRUST_BOUNDARY` 约束的 normalized provisional 与 review packet 语义内容构成；不回放历史 assistant/toolResult 角色或 tool-call ID。Witness 没有 override；代码不在两者之间做语义裁决。Harness 唯一执行 `S=(S0-Δ-)∪Δ+` 并 compact final ranges。发布必须满足 `FINAL_SELECTION ∩ FINAL_HARD_ROOT_PROJECTION = ∅`，且恰好发生 `2 Finalizer + 1 Witness`。Harness 可把 provisional typed selection 与 hard-root projection 的确定性交集作为 mechanical contract blocker 回传，类似 coding agent 的编译错误；它只能指出字段自相矛盾，不能决定应修改 selection、claim exit 或哪项语义结论。任何失败均原样保留 Candidate。

Witness 固定提交 `exclude` 与 `select` 两个必填、非 nullable array lane；`exclude` 可提交 0-3 张彼此独立的反例 card，`select` 可提交 0-1 张，空 lane 精确为 `[]`，不使用 `kind=none` sentinel。每张 card 只 target 一个完整 canonical block，字符串精确为 `段落N`；禁止多 block range 或“前半段/后半段/尾句/其中”等 block 内切片。每张 card 另含一句 premise 和 1-8 个 supporting focus block；direction 由外层 lane 机械派生，不是模型字段。lane 名、card 数量、card 顺序和 supporting block 数量都没有证据权重，不能形成投票。全局 JSON/turn/schema/cross-field 与完整 source 地址合同必须先通过，随后代码才逐 card 对 target 是否出现在 `source_ordered_blocks`、是否属于对应 lane 的一个 `authorized_target_groups` 连续 range、是否位于 `AUDIT_UNIVERSE`，以及 support 是否进入全局 focus 做机械校验；`target_lane=null` block 只可作 support。source 中真实存在但未获动态权限的 card 必须整卡弃权，不能裁剪或修补；同 lane 其他有效 card 与另一 lane 独立保留。空 lane 为 `valid_none`，至少一张有效 card 为 `valid_challenge`，非空且全部 card rejected 才为 `rejected_source_focus`；任一卡 rejected 形成 `partial` coverage，只有双 lane 都完全 rejected 才阻断发布。Finalizer 第二轮必须逐 card 独立核验有效 predicate；rejected card 的 lane/index、range、premise、supporting ID、typed reason 与 error 只能留在 trace 且 `forwarded=false`，Finalizer 只能看到中性 coverage/lane status、card index 和有效 challenge。

Active Pi-native 使用一个共享 `atomic controlling-predicate gate`，先拆 target-own propositions、归一同 block 的具体条件，再按 controlling predicate 的 semantic role 区分 requirement 与 price/proof/procedure/legal/meta/shell/pointer。计算、填写、提交、支付、扣除、赔偿、取消等语法动作若属于 excluded role，不构成 survivor；金额、预算、计费基数、费率和收费标准不得改写为项目物理规模；跨 block 指代只定位 antecedent，不复制其 requirement predicate。性能指标先按衡量对象分类：直接衡量供应商工作/成果的准确性、及时性、完整性或合格性时，即使随后触发扣费也仍是 performance threshold；仅计算价款或费用且不定义履约质量边界时才是 monetary operand。并列 trigger 必须逐 proposition 判断，任一具体履约 alternative 都触发 whole-block survivor veto。Witness 随后执行一次 global card tournament：先按肯定 exclusion mechanism × source-proven peer-bounded partition 分组，同一 root、premise 或连续 remedy/price/proof cluster 最多一张，再按 `source-proven hard-root contradiction > typed/rationale/source exact contradiction > strongest singleton falsifier > ordinary local atom` 排序；同级候选才比较 source 确定性、whole-block 原子确定性、材料性与独立失败覆盖。重复、别处覆盖、内容相似或结果更短不能改变 Owner 或 membership；embedded proof tail 不能伪造独立 Owner root。Cross-reference recovery 必须在 later module atom gate 前完成；来自合格 source 的“遵守/执行 X，详见附件”是 operative incorporation，later 固定匹配 module 不能因处罚或合同式措辞重新继承前一 carrier，但 recovery 只重新开放逐 block atom evaluation，不授予整段 membership。Finalizer 对全部 selected blocks 和 provisional gaps 使用同一 gate，并按 local hard root/heading、excluded-role cluster、excluded-gap 三个风险面完成 terminal scan；heading closure 在 child 终态后按相对层级自底向上重算，只增加 heading 地址，不改变 excluded child。技术附件/制度 intro 确定适用对象并指示执行后续固定规则时是 applicability ATOM。每个 final selected block 必须以 target-own `ATOM` 或合法 `HEADING` closure 为唯一 provenance，Owner、recovery、module、cluster 或相邻 survivor不能成为第三种保留来源。第二轮先冻结 exact `S` 与 sparse typed delta，再压缩记录 delta reason，并对 reason 中 exact verdict 与机械派生 final set 做最终一致性扫描。`owner_reason` 只记录身份与 root→exit，不得塞入 atom 清单；不得用“其余范围均合格”等组摘要替 exact-block closure。上述语义判断全部由模型完成，Harness 不识别 predicate、partition、family 或 cluster。
...
Active Pi-native 的最终集合还必须执行 `symmetric_partition_fixed_point` 与 `semantic_projection_checksum`：每个 selected compact range 都是其内部每个 canonical block 均有 membership 的全称命题，必须寻找 target-own 肯定 exclusion 反例并拆出 holes；每个 excluded gap 都是其内部每个 block 均无 membership 的全称命题，必须寻找 target-own survivor 或真实 heading/body/table closure 反例并恢复。宽 range、组摘要、端点、相邻正例和同 subsection 的其他 survivor 都不能替内部 block 证明。双向攻击结束后先冻结最终 block-ID set `S`，再从 `S0` 与 `S` 唯一生成 typed `Δ-` / `Δ+`；任何已明确判定的 hole 都必须进入 `Δ-`，任何已明确判定的 survivor 都必须进入 `Δ+`。reason 不得再写一份最终 compact ranges 与 typed delta 竞争。该过程使用 phase-specific final-delta schema，但不增加调用、ledger 或代码语义判断。

Active Pi-native typed final-delta fixed point 是 targeted delta 的双向验证，不是重生一份整文答案。第二轮先冻结 provisional block-ID set `S0`；`S0` 不是真值，但是唯一集合编辑基线。先关闭 reason/typed set 自相矛盾，再不依赖 Witness 卡对全部 selected blocks 执行 exclusion closure、对全部 provisional excluded gaps 执行 survivor closure，核验 Witness 后再做一次 terminal exact-block scan，最后投影 delta。每个变化必须由 exact target-own predicate 成立并进入 `Δ-` 或 `Δ+`，第二轮只提交实际变化 run 的 `remove_ranges` / `add_ranges`，Harness 唯一执行 `S=(S0-Δ-)∪Δ+`；不得从新 reason、章节摘要、“其余均合格”或整段分组重新生成 selection。`Δ-` 不得出现在 final selection，`Δ+` 必须全部出现。该过程不增加调用、ledger 或代码语义判断。
...
运行时按角色切片加载 Prompt：Finalizer 只加载 mode-specific 的 `pi-native-semantic-contract.md` 与 `finalizer.md`，并由动态 `TERMINAL_CONTRACT` 和 tool schema 提供当前 run 的两轮字段权限；完整 `pi-native-runtime-contract.md` 仍是 Harness/governance 真源并进入 capability hash，但不整篇注入任一模型。Witness 只加载自包含的 `witness.md`，再接收当前 run 的结构化 provisional ranges/claims、exact JSON schema、严格 source-ordered 的 `REVIEW_FOCUS_SOURCE`，以及 schema-bounded `UNTRUSTED_PROVISIONAL_RATIONALE`；Harness 必须先呈现 focus，再呈现 rationale。Rationale 的唯一用途是让 Witness 看见作者已经实际采用的 claim 并对其做可证伪攻击；它不增加 source evidence、地址权限、card 权重或语义权威。Witness 必须以 source 与实际地址决定结论，不得引用 rationale 作为 premise support，也不得因 rationale 未提及某地址就跳过全局扫描。本文仍是治理和评估真源，但不整篇塞入模型上下文。这样避免其他角色、transport、trace、失败预算和评估说明干扰窄职责判断。
...
- Finalizer 与 Witness 的严格 schema、Witness focus 和 trace；Finalizer `owner_reason` 的机械 schema 硬上限为 1200 字符，`residual_reason` 以 2400 字符为压缩目标、8000 字符为机械硬上限。代码不解释 reason 语义，也不得静默截断超限语义文本；每张 Witness premise 只保留非空白校验与全局 2400-token 预算，不对自然语言字符数设本地 hard gate。Witness 必须返回完整 `exclude`/`select` 双 array lane，空 lane 为 `[]`；exclude 0-3 张、select 0-1 张，每张 card 恰有一个 range、一句 premise 和 1-8 个 supporting IDs。Harness 只接受一次纯 JSON text，以原生 `JSON.parse`、TypeBox 和交叉字段规则严格校验；缺字段、`null`、字符串 `"null"`、空 card、旧 none/primary/secondary、direction 或额外字段全部 fail-closed，不做兼容归一化；
- 在严格 source-ordered 的 `REVIEW_FOCUS_SOURCE` 之后，把上述两个已通过 schema、原文合计最多 9200 字符的 provisional reason 原样 JSON 序列化为 `UNTRUSTED_PROVISIONAL_RATIONALE`，并由既有完整 provisional output reserve 纳入 Witness 容量预检；代码不得摘要、解释、提取关键词、生成攻击方向或根据 rationale 选择 focus，只能附加固定 trust-boundary 文本。Witness 仍只能从可见 source focus 重证 card premise；
...
- 第一轮只对 provisional `run_selections` 执行 run 权限投影：允许非空、单连续授权岛加普通 `OUT` spill 的机械裁剪；全 `OUT`、cross-run、multi-island 或不存在地址必须 fail-closed。第二轮 `run_deltas` 不得 spill：remove 只能引用同 run 的 provisional-selected 地址，add 只能引用同 run 的 provisional-excluded 地址；错误 membership、重复 block/run、空 per-run delta、cross-run、OUT 或不存在地址全部 fail-closed；
- 在任何逐 card 动态判断前，对 Witness 所有 card 的 target range 与 supporting ID 统一做完整 source 地址校验；非法或不存在地址整 Witness fail-closed。随后只按最终实际序列化的 focus 与 audit universe 做原子权限校验：target 必须出现在 `source_ordered_blocks`，`exclude` target 必须完整位于一个 `authorized_target_groups.exclude` range，`select` target 必须完整位于一个 `authorized_target_groups.select` range，全部 target 必须属于 `AUDIT_UNIVERSE`；`target_lane=null` block 只可作 support，support 只须真实存在且进入全局 focus。source 中存在但 focus/lane/group/universe 越权时拒绝整张 card，不裁剪、不补洞、不把同向地址过滤成残余 challenge；同 lane 与另一 lane 的其他有效 card继续，只有双 lane 都完全 rejected 才使 Witness fail；
- 仅对权限有效的 Witness card 做 claimed/unclaimed 机械分区；rejected 原始 card、lane/index 和 typed reason 只进入 trace 并标记 `forwarded=false`，不得把其 range、premise、support、error 或错误地址转发给 Finalizer；
- 仅按 typed provisional hard-root claim 与地址连续性，为 Witness focus 加入每个 root 和非 EOF exit 的目标地址及前后最多两个实际 source block，并为其 projection 连续岛加入固定首尾各两个 block，使 Owner 反例只能引用实际看过的有界 source；全部 focus block 去重后在 `source_ordered_blocks` 中按 `block_id` 严格升序、每个 block 和正文只出现一次，再以 `target_lane=exclude|select|null` 与 `authorized_target_groups` 分离 source 呈现和 target 权限。Harness 必须先序列化 source focus，再序列化 `UNTRUSTED_PROVISIONAL_RATIONALE`。固定 256-block / 180000-character 预算容纳得下时完整展示全部 selected target islands，超预算时仅按 block ID 与地址连续性在各岛间轮询，并用确定性的 breadth-first recursive-midpoint 顺序均匀覆盖，不能让早期长岛饿死后续岛；该窗口、权限和调度都不证明 claim、Owner、membership 或修复方向；
- 按 `S=(S0-Δ-)∪Δ+` 机械派生 final selection，校验其与 final hard-root projection 零交集，并在任何失败时保留 Candidate。
...
Active Pi-native 每个 case 固定最多三次 provider call：GLM Finalizer provisional、Doubao 2.0 Pro Witness、同一个 GLM Finalizer final。成功发布必须恰好三次；失败路径可提前终止。Finalizer provider 边界最多两次，首轮无合法 provisional 时立即停止，第二轮后无条件停止；每轮必须恰好一个目标工具调用，owner reason 硬上限 1200 字符，residual reason 目标 2400、硬上限 8000 字符。普通 text 若出现只能作为不参与结果的 trace-only auxiliary output；thinking、未知或额外工具仍拒绝。Witness 无论是否发现反例都只调用一次。无 retry、第四次调用、投票、best-of-N、逐 block ledger 或自由读搜 loop。两次 Finalizer 和 Witness 均关闭 thinking；Witness 最多 2400 output tokens，使用 `tools=[]`，provider payload 发送 `response_format={"type":"json_object"}` 与 `thinking={"type":"disabled"}`。Prompt 内提供 exact JSON schema，provider 只负责 JSON object transport；唯一接受路径仍是单一 assistant text 的严格 `JSON.parse`、同一 TypeBox schema 和 per-card 交叉字段校验，provider JSON mode 不构成语义裁决或本地合同替代。

调用前必须用 answer-free、确定性的上下文估算做容量预检。provider、capacity、timeout、abort、budget、全局 JSON/turn/schema/cross-field、完整 source 地址、双 lane 全部动态授权失败、调用数或最终一致性失败均不应用语义改写，Candidate 原样保留并标记 degraded。单张 card 的 focus/lane/group/universe 动态越权只把该原始 card 机械降为 `rejected_source_focus`，不增加调用、不触发 retry，也不阻断同 lane/另一 lane 的有效 card 与第三次 Finalizer；成功结果必须显式标记 `partial` coverage。上述非空单岛普通 `OUT` spill 的最小权限投影只是删除模型无权选择的地址，不是 range failure、语义补丁或答案裁决；其他越权形态仍失败关闭。上游 Candidate 成本与新增 Agent 成本分开报告。
