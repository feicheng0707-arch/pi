...
运行时按角色切片加载 Prompt：Finalizer 只加载 mode-specific 的 `pi-native-semantic-contract.md` 与 `finalizer.md`，并由动态 `TERMINAL_CONTRACT` 和 tool schema 提供当前 run 的两轮字段权限；完整 `pi-native-runtime-contract.md` 仍是 Harness/governance 真源并进入 capability hash，但不整篇注入任一模型。Witness 只加载自包含的 `witness.md`，再按顺序接收 strict source-ordered `REVIEW_FOCUS_SOURCE`、`MECHANICAL_TARGET_AUTHORIZATION`、包含 projected/no-projection claims 的 `PROVISIONAL_HARD_ROOT_CLAIMS` 与 exact JSON schema；不接收 provisional narrative rationale。Witness 必须以 source 与实际地址决定结论。本文仍是治理和评估真源，但不整篇塞入模型上下文。这样避免其他角色、transport、trace、失败预算和评估说明干扰窄职责判断。
...
- Finalizer 与 Witness 的严格 schema、Witness focus 和 trace；Finalizer `owner_reason` 的机械 schema 硬上限为 1200 字符，`residual_reason` 以 2400 字符为压缩目标、8000 字符为机械硬上限。代码不解释 reason 语义，也不得静默截断超限语义文本；每张 Witness `source_conclusion` 只保留非空白校验与全局 2400-token 预算，不对自然语言字符数设本地 hard gate。Witness 必须返回完整 `remove_from_provisional` / `add_to_provisional` 双 array，空数组为 `[]`；前者 0-3 张、后者 0-1 张，每张 card 恰有一个 range、一句 source conclusion 和 1-8 个 supporting IDs。Harness 只接受一次纯 JSON text，以原生 `JSON.parse`、TypeBox 和交叉字段规则严格校验；缺字段、`null`、字符串 `"null"`、空 card、旧 exclude/select/attacked_premise/none/primary/secondary、direction 或额外字段全部 fail-closed，不做兼容归一化；
- Witness 输入不得携带 provisional `owner_reason` / `residual_reason` narrative rationale。Harness 必须先序列化严格 source-ordered 的 `REVIEW_FOCUS_SOURCE`，随后独立序列化 `MECHANICAL_TARGET_AUTHORIZATION` 与包含 projected/no-projection 两类 claim 的 `PROVISIONAL_HARD_ROOT_CLAIMS`；代码不得摘要、解释、提取关键词、生成攻击方向或根据 claim 内容选择 focus；
...
- 在任何逐 card 动态判断前，对 Witness 所有 card 的 target range 与 supporting ID 统一做完整 source 地址校验；非法或不存在地址整 Witness fail-closed。随后只按最终实际序列化的 focus 与 audit universe 做原子权限校验：target 必须出现在 `source_ordered_blocks`，`remove_from_provisional` target 必须完整位于对应 authorization range，`add_to_provisional` target 同理，全部 target 必须属于 `AUDIT_UNIVERSE`；authorization 外的 visible block 只可作 support，support 只须真实存在且进入全局 focus。source 中存在但 focus/group/universe 越权时拒绝整张 card，不裁剪、不补洞、不把同向地址过滤成残余 challenge；同 internal lane 与另一 lane 的其他有效 card 继续，只有双 lane 都完全 rejected 才使 Witness fail；
