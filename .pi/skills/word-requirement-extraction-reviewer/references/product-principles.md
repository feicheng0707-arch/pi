...
运行时按角色切片加载 Prompt：Finalizer 只加载 mode-specific 的 `pi-native-semantic-contract.md` 与 `finalizer.md`，并由动态 `TERMINAL_CONTRACT` 和 tool schema 提供当前 run 的两轮字段权限；完整 `pi-native-runtime-contract.md` 仍是 Harness/governance 真源并进入 capability hash，但不整篇注入任一模型。Witness 只加载自包含的 `witness.md`，再接收当前 run 的有界 focus、结构化 provisional ranges/claims、exact JSON schema，以及 schema-bounded `UNTRUSTED_PROVISIONAL_RATIONALE`。Rationale 的唯一用途是让 Witness 看见作者已经实际采用的 claim 并对其做可证伪攻击；它不增加 source evidence、地址权限、card 权重或语义权威。Witness 必须以 source 与实际地址决定结论，不得引用 rationale 作为 premise support，也不得因 rationale 未提及某地址就跳过全局扫描。本文仍是治理和评估真源，但不整篇塞入模型上下文。这样避免其他角色、transport、trace、失败预算和评估说明干扰窄职责判断。
...
- 把上述两个已通过 schema、原文合计最多 9200 字符的 provisional reason 原样 JSON 序列化为 `UNTRUSTED_PROVISIONAL_RATIONALE`，并由既有完整 provisional output reserve 纳入 Witness 容量预检；代码不得摘要、解释、提取关键词、生成攻击方向或根据 rationale 选择 focus，只能附加固定 trust-boundary 文本。Witness 仍只能从可见 source focus 重证 card premise；
...
- 在任何逐 card 动态判断前，对 Witness 所有 card 的 target range 与 supporting ID 统一做完整 source 地址校验；非法或不存在地址整 Witness fail-closed。随后只按最终实际序列化的 focus 与 audit universe 做原子权限校验：`exclude` target 必须完整位于一个 selected group，`select` target 必须完整位于一个 excluded group，全部 target 必须属于 `AUDIT_UNIVERSE`，support 只须真实存在且进入全局 focus。source 中存在但 focus/state/group/universe 越权时拒绝整张 card，不裁剪、不补洞、不把同向地址过滤成残余 challenge；同 lane 与另一 lane 的其他有效 card继续，只有双 lane 都完全 rejected 才使 Witness fail；
...
- 仅按 typed provisional hard-root claim 与地址连续性，为 Witness focus 加入每个 root 和非 EOF exit 的目标地址及前后最多两个实际 source block，并为其 projection 连续岛加入固定首尾各两个 block，使 Owner 反例只能引用实际看过的有界 source；全部 focus block 去重后只按 provisional state 与连续 block ID 分成 selected/excluded 两类岛，每个 block 和正文只出现一次；固定 256-block / 180000-character 预算容纳得下时完整展示全部 selected islands，超预算时仅按 block ID 与地址连续性在各岛间轮询，并用确定性的 breadth-first recursive-midpoint 顺序均匀覆盖，不能让早期长岛饿死后续岛；该窗口、分组与调度都不证明 claim、Owner、membership 或修复方向；
