...
- Finalizer 第一次或第二次 provider/schema/tool/地址权限失败，包括 owner reason 超过 1200 字符或 residual reason 超过 8000 字符：Candidate 原样保留，`reviewDegraded=true`。2400 字符只是 residual 压缩目标，不是 correctness gate；Harness 不截断超限 reason。单岛普通 `OUT` spill 经机械投影后不属于失败；其 raw submission、canonical decision 和被裁地址必须可审计。
...
- 调用前用 answer-free 确定性上界同时估算 Finalizer 第二轮最坏 context 与 Witness 最坏 context；任一冻结模型容量不足都在第一次语义调用前失败关闭。Finalizer 每轮输出上限 8000 tokens，owner reason 硬上限 1200 字符，residual reason 目标 2400、硬上限 8000 字符；Witness 2400 tokens。run 总 input/output/reasoning 上限分别为 720000/20000/1000 tokens。
