...
3. **Outside-carrier atom gate**
   ...
   - 对孤立的成交前包装做 `response_wrapper_survival_attack`：先去掉“在响应文件中填写、提交、自行承诺、确认或声明”等包装；若同一不可分 block 仍直接要求成交供应商实施、配置、保障安全或质量、交付、验收、服务或达到具体结果，恢复该 block。只有边界完整的响应/承诺 carrier，或可安全分离且余下仅为证明、表态、无偏离或材料提交的 atom，才写入 `outside_carrier_excluded_ranges`。
...
- `response_wrapper_survival_attack`：四类载体外的孤立成交前提交/承诺/确认/声明包装，剥离后是否仍留下不可分的实施、安全、质量、交付、验收、服务或结果 duty；有则恢复，不能仅凭包装词删除。
