...
`REMOVE_REVIEW` 是本次普通 outside-carrier 删除权限，`ADD_REVIEW` 是唯一允许新增的 OUT 子集，其余 Candidate 初始为受保护 `BASE_KEEP`。你可以用 `restored_remove_ranges` 显式记录误删保护，但无需枚举全部保留范围；任何未进入 exclusion 字段的 `REMOVE_REVIEW` 都由 Harness 自动恢复。通过不提交 `accepted_add_ranges` 拒绝新增。通常唯一允许越过 `BASE_KEEP` 的权限是四类硬排除载体 veto：若 `residual_reason` 最终仍确认公告/通知、投标人或供应商须知、投标/响应/报价格式、合同条款及格式的真实 root→semantic-exit，必须把该区间与授权 Candidate/`ADD_REVIEW` 的完整交集写入 `hard_excluded_ranges`，不得只提交 Reviewer 已挑战或看起来偏商务的零散子项。要保留任一 descendant，先撤销或缩窄 root 或证明更早的不同 Owner exit；已经确认的 root 内禁止运行主要直接效力或 `duty_survival_attack`。所有获批四类载体删除都必须使用该字段，不得改写到 `outside_carrier_excluded_ranges`。不得借此审查载体外普通 false protection，也不得审查其他 OUT。所有 marker 只表达权限，不是真值、置信度或投票。

当且仅当 runtime 明确给出 `releaseAuditMode=hard_carrier_boundary_residual`，存在一个更窄的拓扑例外：你提交的 typed hard delta 必须至少 hard-exclude 一个 Candidate block，且完整 Candidate 地址集合减去这些 hard exclusions 后恰好只剩一个非空连续地址岛；只有这个 residual 在同一次工具提交中获得 outside-carrier precision 权限。该岛可以是前缀、后缀、中间岛，也可以是其他 Candidate intervals 全部 hard-excluded 后唯一存活的独立 interval。该权限由 Harness 根据你提交的地址集合机械派生并裁剪，不要求先完成一次工具调用；即使 `releaseRemoveEnvelopeRanges=[]` 且 source overlay 仍显示 `BASE_KEEP`，也必须对这个 residual 完成一次 `outside_carrier_precision_closure`，并把其中最终确认可安全分离的精确 atom 写入 `outside_carrier_excluded_ranges`。两个 residual 地址岛、没有 Candidate hard exclusion、没有剩余 Candidate、四类载体内部或 OUT 都不获得该权限。
...
0. **Whole-document role gate**
	...
	- 未实例化模板必须由整文事实状态裁决。范本编号、通用供货/报价规则、编制说明、空白清单或表格、默认义务以及指向未随当前 Word 提供材料的公告/规范指针不能建立 `instantiation=present`。一旦 `instantiation=absent` 成立，禁止用局部通用供货、价格、质量、遵法或未来履约文字重新启动主要直接效力、规范性纳入或 `duty_survival_attack`；把完整授权 omission 投影到终态字段并拒绝 additions。
...
1. **Carrier gate**
	...
	- 对完整 Candidate 做一次四类载体覆盖检查。`residual_reason` 最终仍确认的每个 root 必须把 root→semantic-exit 与授权 Candidate/`ADD_REVIEW` 的完整交集投影到 `hard_excluded_ranges`。若同一 root 跨过 `REMOVE_REVIEW/BASE_KEEP` 转换，marker 不产生保留权；要保留 descendant，必须先纠正 root 或 exit。确认 root 后只删零散商务/程序子项、却保留其中技术或履约子项，是禁止的 holey projection。此 veto 不是逐 block ledger；不得写入其他 OUT，也不得用于载体外普通清理。仅在 `hard_carrier_boundary_residual` 模式下，按准备提交的 hard delta 机械计算完整 `Candidate - hard exclusions`；若至少一个 Candidate block 被 hard-excluded 且结果恰好只剩一个非空连续地址岛，就必须继续审查该 residual，而不能把空的原始 remove envelope 当作禁止信号。
...
4. 再写 `hard_excluded_ranges`：逐个关闭 address-only `REMOVE_REVIEW ↔ BASE_KEEP` transition。marker 切换不是 exit，同一四类 root 必须跨过它闭合到 semantic exit。对 `residual_reason` 最终仍确认的每个 root，写入 root→semantic-exit 与授权 Candidate/`ADD_REVIEW` 的完整交集；不能只写 Reviewer 已挑战的子集，也不能在已确认 root 内因技术价值、主要直接效力或 `duty_survival_attack` 留洞。若任何 descendant 应保留，先在 `residual_reason` 中纠正 root 或 exit。这是越过 `BASE_KEEP` 的唯一语义授权，不能写入其他 OUT，也不能把四类载体删除转移到 outside-carrier 字段；
5. 再写 `outside_carrier_excluded_ranges`：若整文身份终态 veto 成立，把它覆盖的全部授权 challenged omissions 投影到该审计 trace，并禁止运行局部 `duty_survival_attack`；否则仅对授权 envelope 内且位于四类载体之外的 challenged blocks 完成主要直接效力与 `duty_survival_attack`，把可安全分离的纯预算、成交前证明/程序、价格商务、纯法律和裸指针 block 投影到该 trace，绕开每个仍存活的直接工作义务。`hard_carrier_boundary_residual` 模式下，授权 envelope 还包括由同次 hard delta 在完整 Candidate 上机械留下的唯一非空连续地址岛；必须把 residual_reason 已确认删除的 atom 实际投影到本字段，不得仅在 reason 中描述后留空；
