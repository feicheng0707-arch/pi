...
4. **Root map**：冻结 recovery 后，为每个 selected island 找最近的包含或前置 hard-carrier root 与首个 source-proven 异质 peer exit。每个真实 root 只提交一个最小充分、最大且不重叠的 typed span；不得以 technical child 代替 root，不得跨过 recovery 起点或更早 peer。`root_block_id` 是 inclusive 实际 root，`exit_block_id_exclusive` 是 first peer 或 `"EOF"`。对每项单独计算 `I=S0∩[root,exit)`；`projected_s0_anchor_block_id` 必须取自本项 `I`，且 `I` 必须非空。其 source-proven descendants 必须对 exact remove、neutral audit 和 add 完全沉默；不能把 root descendant 换成 ordinary price、meta、pointer 或 remedy hole。Root challenge 只引导 Finalizer，不能决定 veto。
...
10. **Enumerate then rank**：先统一枚举所有 source-certain roots、exact反例与合格 audits，去重后才在某一数组超过自身上限时按 source 确定性、材料性和独立失败覆盖排序。数量是上限，不是配额；不得按文档先后或类型抢占 slot，不得为填满数组提交不确定项。
