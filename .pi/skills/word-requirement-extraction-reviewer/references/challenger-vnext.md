# Candidate-S0 独立 Challenger v33
...
7. **Audit-first routing**：序列化 exact card 前先冻结 source-certain typed audits。`recovery_boundary_scope` 只承载第 3 步已成立的一个完整 later module，从 intro/title 到真实 peer exit 的全部 Candidate scope；不得混入 earlier root、其他 module 或无关 run。`mixed_atomic_scope` 只承载 root/recovery 外同一最小 peer-bounded ordinary scope 的 competing atomic roles 或 heading risk。每种最多一项，互不占位且零重叠；超过 96 blocks 时省略，不裁剪、拼接或改类型。同质 cluster、地址连续或 exact 候选较多不能自行建立 audit。
8. **Audit sparse burden**：audit 不是整段删除结论、保留票或不确定项收容器。完整 scope 内已肯定的 source-certain sparse holes 必须各自进入 exact 候选集合；supporting IDs 不能代替 exact hole。全部重叠 exact targets 的并集必须是该完整 audit target set 的严格子集，不能用 exact 通道预删、缩窄或替代 audit。Recovery scope 的 holes 不能把它改标为 mixed；hard-root descendants 没有 mixed-audit 资格。容量未超限时，每个已肯定 hole 都必须作为独立 singleton card 提交；只有超限后才允许统一排序取舍。
9. **Sparse residual enumeration**：排序前在同一次完整扫描中列出：tentative body/table holes 后失去 final descendant 的 heading；excluded-role heading 与 mixed child 的不对称；剥离后果后只剩抽象事件、无 target-own survivor tuple 的 block；actor reset 后只约束 source-proven peer actor 的 block；及其他 source-certain singleton holes。全部完成后才去重，不得按文档顺序占位。每个 target 仍须通过第 5 步 survivor falsification；撤销 target 后重跑第 6 步 fixed point。
10. **OUT omissions**：add 只表达 `S0` 外、完整 source 肯定具有 `ATOM|HEADING` provenance 的真实遗漏。Recovery、边界独立、内容详细、Candidate 赞同或防止 Finalizer 误删都不是 add；任何 `S0` 地址不得进入 add。
11. **Enumerate then rank**：先统一枚举 source-certain exact remove 集合 `E-`、exact add 集合 `E+` 与合格 audits。exact 只按“同方向 + 同 block 地址”去除完全重复项；相邻、连续、同 conclusion、同机制或同 cluster 都不重复。`|E-|<=8` 时逐目标提交全部 singleton remove cards，超限才按 source 确定性、材料性和独立失败覆盖取前八张；`|E+|<=2` 时全部提交，超限才取前两张。audit 服从各自 typed 上限。不得按文档顺序、错误类型、lane 或共同 premise 预留、合并、抢占 slot，也不得为填满数组提交不确定项。内部 root map 不参与排序。
...
- Exact card 的 `supporting_block_ids` schema/transport hard cap 是 4；audit partition 的 transport hard cap 仍是 32、协议 cap 仍是 12。Harness 不截断、不挑选、不保留前 N 个。
- 每个 exact partition 只含 `target_ranges, source_conclusion, supporting_block_ids`。`target_ranges` 恰有一个 singleton `段落N`；table block 同样使用其唯一地址。禁止多 block range、多 target、block 内切片或合并相邻目标。`source_conclusion` 只证明目标自身的一个 source-certain premise，最多 160 字符；support 必须包含 target，合计 1-4 个真实唯一的最小充分地址。完整 `S0` 闭包是静默工作队列；不得为未挑战 block 输出 verdict 或 evidence ledger。
...
提交前静默确认：全部 run 已完整消费；recovery 先于 root inheritance；内部 roots 使用 inclusive actual root 与 exclusive first peer/EOF、spans 去重且 descendants ordinary-silent，但没有任何 root 字段进入输出；每个 exact partition 恰有一个 singleton target、同方向 target 地址唯一、remove 无 survivor、add 不只靠 boundary；若 source-certain exact 候选未超过对应 8/2 上限，submitted target set 与完整候选集合一致，只有超限时才允许按冻结排序舍弃，相邻或同 premise 不能在容量未满时造成遗漏；audit kind、完整 scope、预算、零重叠与 exact-overlap strict subset 均成立；exact supporting IDs 为 1-4 个且包含 target，audit supporting IDs 为 1-12 个，全部真实唯一且没有代替 target。不得依赖 Harness 截断，因为 Harness 不会截断。
