...
3. 在提交任何 atomic partition 前，对 `S0` 的每个 selected island 先定位其最近的包含或前置 source-function root，并确定真实 peer exit。四类 hard carrier 的完整 `root→peer exit/EOF` 由 terminal Finalizer 在全 `S0` 独占提交 typed root veto；你必须识别这些边界以避免错误 recovery 或 atom 判断，但不得把“属于四类 hard carrier”作为 exact remove partition 的唯一 premise，也不得用有限 remove slots重复枚举其 descendants。不得从 Candidate 反推 source，也不得因为 Candidate 已成熟而降低 source 证据门槛。
4. exact remove partition 只攻击四类 hard carrier 之外、可由目标 block 自身肯定裁决的原子或 heading hole。你自己判定仍由四类 hard root 支配的任何 block 都不得占用 exact remove 或 mixed audit；前置 carrier span保持沉默，交给 Finalizer root veto。只有完整 source 已独立建立 recovery 的累计资格、但 recovery module 内部存在 mixed atomic membership 时，才可把 recovery module 从独立 intro 到真实 peer exit 的最小范围放入 recovery audit。若一个合格 Owner 中仍被选择的最小 source-bounded scope明显混合 `ATOM|HEADING` survivor 与 excluded blocks，但稀疏 holes 无法在不做逐 block ledger 的情况下可靠枚举，可提交 neutral remove-audit scope，让独立 Finalizer 在该 scope 内形成 exact subset。audit 不是删除结论、保留票或不确定项收容器，且不得与任何 exact remove target 重叠；已经 exact 定位的地址不需要再次 audit。
5. audit slot 分两条独立 typed 通道检查：普通 selected island 内的 mixed heading/body、价格/资格/程序 wrapper 与 atomic holes 使用 `audit_kind=mixed_atomic_scope`；经 recovery、operative incorporation 或边界恢复后被选择的 later module，从 module intro 到真实 peer exit/尾部的 meta、pointer、other-actor、抽象后果或开放式 epilogue风险使用 `audit_kind=recovery_boundary_scope`。后一类型同时要求 Finalizer独立攻击任何仍跨过该 scope 的前置 hard-root continuity，不是预先证明 recovery 成立。两类都成立时各保留一个最材料、最小的 scope，普通 mixed island 不得挤占 recovery-module audit；某类无 source-grounded mixed risk 时不为它占 slot。
6. root map 完成后不得立即结束，也不得因为 Finalizer 会处理 hard carriers 而跳过其余 Candidate。对每个不由四类 carrier 完整支配的 selected island，从首 block 到末 block执行一次 `remaining-island universal falsification`：把“岛内每个 block 都有 ATOM/HEADING provenance”作为待证伪全称命题，检查 heading transition、pointer/meta/price/proof/procedure child、actor 切换、后果 tail 与末端 epilogue。若同一真实 peer-bounded scope 内出现多个可分离 body holes、bare pointer 或可能随 child 删除变空的 heading，优先提交覆盖该局部闭合的一个 `mixed_atomic_scope`，不能只枚举 body holes后把 heading 留在 envelope 外。只要 source 同时证明 survivor 与至少一个可分离风险 block，必须把覆盖该失败机制的最小 scope放入对应 audit；`remove_audit_partitions=[]` 只允许在全部 eligible islands 都完成该闭合且无 mixed risk 时提交。
...
8. 统一枚举全部有资格的反例后按 source 确定性、材料性和独立失败覆盖排序，再提交有限 partitions。不得按文档先后或类型先占名额。`MECHANICAL_AUDIT_BUDGET` 给出完整 Candidate runs 的 answer-free block count 与固定预算；两条 typed audit 通道各最多一项、各不超过 96 blocks，合计去重上限 192 blocks。必须取足以覆盖同一局部失败机制的最小范围，不得机械重开完整 Candidate，也不得把仍由 hard-root veto 处理的前置 carrier fragments 塞进 recovery audit。
