...
Candidate、`IN/OUT`、`REMOVE_REVIEW`、`ADD_REVIEW`、`BASE_KEEP` 只表示已有选择与本次机械权限，不是真值、投票或语义边界。`REMOVE_REVIEW` 是唯一普通载体外 Candidate 删除子集，`ADD_REVIEW` 是唯一可新增的 OUT 子集，其余 Candidate 是受保护 `BASE_KEEP`。通常只有模型独立证明某个 Candidate block 位于四类硬排除载体 root→semantic-exit 内，并把同一省略 block 写入 `hard_excluded_ranges`，才可越过 `BASE_KEEP`。若 `buyer_issued + present` Reviewer 的普通 removal 覆盖完整 Candidate，Harness 在 Release 前撤销该 ordinary envelope；此时全部 Candidate 初始为 `BASE_KEEP`。Release 提交的 hard exclusion 只有在机械上占满一个连续 Candidate interval 的完整前缀或完整后缀时，才使另一侧 residual 获得仅本 interval 的 outside-carrier precision 权限；没有完整 hard boundary 就没有 residual 权限。结构图只提供同源 body 顺序、样式、outline、编号、字号、粗体、对齐、分页、表格规模和 bounded source prefix；缺失节点不是负面证据。Harness 只把与 `REMOVE_REVIEW ∪ ADD_REVIEW` 相交的现有结构行在固定预算内按地址重列为 challenged-side focus，并可列出有界的 address-only `REMOVE_REVIEW ↔ BASE_KEEP` 连续 run 转换；完整 source 与完整 answer-free 结构图仍用于一次 Candidate 全域四类载体 sweep。这些导航和 residual derivation 都不读取标题含义、不产生新证据、Owner 标签或 keep/remove 结论。
