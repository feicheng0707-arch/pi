...
对四类载体外的孤立成交前包装执行 `response_wrapper_survival_attack`：单个 block 出现“在响应文件中填写/提交/承诺/确认/声明”不能独立建立响应载体或触发 subsection-level Stage Owner。先剥离包装；若同一不可分 block 仍有具体实施、资源、安全、质量、交付、验收、服务或结果 duty，必须保留。只有边界完整的响应/承诺 root，或可安全分离且只承担成交前证明/表态的 atom，才排除。
...
Candidate、`IN/OUT`、`REMOVE_REVIEW`、`ADD_REVIEW`、`BASE_KEEP` 只表示已有选择与本次机械权限，不是真值、投票或语义边界。`REMOVE_REVIEW` 是唯一普通载体外 Candidate 删除子集，`ADD_REVIEW` 是唯一可新增的 OUT 子集，其余 Candidate 是受保护 `BASE_KEEP`。Release 只有一个 Candidate 全域例外：模型独立证明某个 Candidate block 位于四类硬排除载体 root→semantic-exit 内，并把同一省略 block 写入 `hard_excluded_ranges`；除此之外不得越过 `BASE_KEEP`。结构图只提供同源 body 顺序、样式、outline、编号、字号、粗体、对齐、分页、表格规模和 bounded source prefix；缺失节点不是负面证据。Harness 只把与 `REMOVE_REVIEW ∪ ADD_REVIEW` 相交的现有结构行在固定预算内按地址重列为 challenged-side focus，并可列出有界的 address-only `REMOVE_REVIEW ↔ BASE_KEEP` 连续 run 转换；完整 source 与完整 answer-free 结构图仍用于一次 Candidate 全域四类载体 sweep。这些导航不读取标题含义、不产生新证据、Owner 标签或 exit。
...
Release 阅读完整 source，对 `REMOVE_REVIEW ∪ ADD_REVIEW` 完成 bounded patch 裁决，并额外执行一次 Candidate 全域四类硬排除载体 root→semantic-exit sweep。`hard_excluded_ranges` 可投影任意 Candidate 或 `ADD_REVIEW` 中已证明属于四类载体、且准备从 final 省略的 block；这是越过 `BASE_KEEP` 的唯一权限。`outside_carrier_excluded_ranges` 仍只投影授权 envelope 内、经过 `duty_survival_attack` 后可安全分离的载体外 non-requirement atoms。两个字段都不是 ledger。`final_ranges` 是唯一权威完整集合：Harness 只允许 Candidate 或 challenged add；省略的 `BASE_KEEP` 若未同时出现在 `hard_excluded_ranges` 就机械恢复，并把 exclusion 字段裁剪到各自权限与最终未选集合后去重。Harness 不识别 Owner，不得用 reason 或字段内容补写语义。
