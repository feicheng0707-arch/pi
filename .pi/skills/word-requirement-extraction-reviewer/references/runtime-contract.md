...
孤立的委托代理、公开招标、欢迎投标或邀请参与前言不能建立跨章节公告 Owner。下一处 source-proven 平级采购内容、对象、范围、地点、工期、质量、技术要求、图纸或清单 root 应重新开启 Owner；只有同一 uninterrupted notification root 仍继续展开资格、文件获取、递交截止、发布媒介或联系方式时，公告 Owner 才能越过该位置。
...
Candidate、`IN/OUT`、`REMOVE_REVIEW`、`ADD_REVIEW`、`BASE_KEEP` 只表示已有选择与本次机械权限，不是真值、投票或语义边界。`REMOVE_REVIEW` 是唯一普通载体外 Candidate 删除子集，`ADD_REVIEW` 是唯一可新增的 OUT 子集，其余 Candidate 是受保护 `BASE_KEEP`。通常只有模型独立证明某个 Candidate block 位于四类硬排除载体 root→semantic-exit 内，并把同一省略 block 写入 `hard_excluded_ranges`，才可越过 `BASE_KEEP`。`BASE_KEEP` 只阻止普通载体外清理；若 `residual_reason` 最终仍确认一个实际四类 root，则该 root 到 semantic exit 内所有授权 Candidate/`ADD_REVIEW` blocks 都必须进入 `hard_excluded_ranges`。要保留任一 descendant，必须先撤销或缩窄 root 或证明更早的不同 Owner exit，不能在已确认 root 内运行主要直接效力或 `duty_survival_attack`。若 `buyer_issued + present` Reviewer 的普通 removal 覆盖完整 Candidate，Harness 在 Release 前撤销该 ordinary envelope；此时全部 Candidate 初始为 `BASE_KEEP`。Release typed hard delta 至少删除一个 Candidate block，且完整 Candidate 减去 hard exclusions 后恰好只剩一个非空连续地址岛时，该岛才获得 outside-carrier precision 权限；它可以是前缀、后缀、中间岛，或其他 Candidate intervals 已全部 hard-excluded 后唯一存活的独立 interval。两个 residual 岛、没有 Candidate hard exclusion 或没有剩余 Candidate 都不授权。结构图只提供同源 body 顺序、样式、outline、编号、字号、粗体、对齐、分页、表格规模和 bounded source prefix；缺失节点不是负面证据。Harness 只把与 `REMOVE_REVIEW ∪ ADD_REVIEW` 相交的现有结构行在固定预算内按地址重列为 challenged-side focus，并可列出有界的 address-only `REMOVE_REVIEW ↔ BASE_KEEP` 连续 run 转换；完整 source 与完整 answer-free 结构图仍用于一次 Candidate 全域四类载体 sweep。这些导航和 residual derivation 都不读取标题含义、不产生新证据、Owner 标签或 keep/remove 结论。
...
`hard_carrier_boundary_residual` 的机械授权在完整 Candidate 地址集合上一次计算：先确认至少一个 Candidate block 被 hard-excluded，再计算 `Candidate - hard exclusions`。结果必须恰好是一个非空连续地址岛；所有其他 Candidate blocks，包括其他 Candidate intervals 中的 blocks，都必须已经 hard-excluded。结果为空或分成两个及以上地址岛均不授权。

## 终态

Release 阅读完整 source，先在 `hard_carrier_reason` 中对每个连续 Candidate interval 建立四类硬排除载体 root→semantic-exit 假设，再在 `residual_reason` 中反向攻击该假设并完成 bounded patch 裁决；两个 reason 必须位于所有 range 字段之前，后者可以纠正前者。随后才投影 typed delta。`restored_remove_ranges` 是可选显式保护 trace；`hard_excluded_ranges` 必须等于最终仍确认的四类 root→semantic-exit 与授权 Candidate/`ADD_REVIEW` 的完整交集，不能只投影零散子项；`outside_carrier_excluded_ranges` 通常只投影 `REMOVE_REVIEW ∪ ADD_REVIEW` 内、经过 `duty_survival_attack` 后可安全分离的载体外 non-requirement atoms，在 `hard_carrier_boundary_residual` 模式下还可投影由同次 hard delta 在完整 Candidate 上机械留下的唯一非空连续地址岛；`accepted_add_ranges` 只批准 `ADD_REVIEW`。四个字段都不是 ledger。Harness 机械以显式 restoration 和 accepted add 为优先级，只执行两个 exclusion 字段明确授权的删除，并把其余 `REMOVE_REVIEW` 自动恢复，再固定派生 `final = Candidate - authorized hard exclusions - authorized outside-carrier exclusions + accepted challenged additions`。Harness 不识别 Owner，不得用任一 reason 或字段内容补写语义。
