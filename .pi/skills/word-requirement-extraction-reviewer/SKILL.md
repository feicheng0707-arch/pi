...
The capability uses one candidate-aware Doubao residual Reviewer. A Reviewer pass ends after one model call. A material challenge invokes one independent Gemini Release that sees the complete source and a mechanically bounded patch but not the Reviewer narrative, evidence leads, or history. Exact removal exposes only the submitted remove ranges. A broad mixed Candidate may be expressed sparsely as protected Candidate ranges; in `candidate_complement` mode the Extension marks the mechanical complement as `PROPOSED_REMOVE`, marks the remaining Candidate as adversarial `AUDIT_KEEP`, and keeps the complete Candidate as the bounded removal envelope. Release reviews that patch instead of regenerating extraction and submits one authoritative complete `final_ranges` set. The Extension only intersects that set with mechanical add/remove permissions and diffs it against Candidate; it does not interpret the reason or make a semantic correction. The fixed cross-model second role exists to break same-model common-mode bias; it is not a vote or best-of-N. There is no third call, retry, block ledger, or code-side semantic repair.
