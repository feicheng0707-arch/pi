...
The active route is one Pi Agent loop with a fixed `GLM provisional -> Doubao 2.0 Pro Witness -> same GLM final` sequence. The first GLM tool call is provisional and returns a Harness review packet without terminating the loop. The second GLM call runs in a fresh deterministic neutral-replay provider context built from the original immutable user input plus the normalized provisional submission and Harness review packet; historical assistant/toolResult roles and tool-call IDs are not replayed. Witness independently returns one strict direct-JSON object with two required, non-null lanes: `exclude` and `select`. Each lane contains its strongest counterexample for that direction or a complete `kind=none` object; direction is mechanically derived from the lane and is not a model field. Lane names, non-empty lane count, and supporting-block count carry no authority, and the Witness cannot override the Finalizer. Each non-empty lane may cite 1-8 supporting focus blocks. That evidence-address allowance does not change the bounded source-quote limits, provider-call count, or Harness semantic boundary. A Witness premise should remain one compact sentence, with roughly 192 Chinese characters as an advisory target rather than a local schema limit; the hard resource bound is the single 2400-token Witness output cap. An `exclude` lane requires an affirmative exclusion predicate in the target source; repetition, redundancy, coverage elsewhere, or a shorter/neater result alone cannot authorize it. The second GLM tool call is the only publishable semantic decision.
...
Any provider, timeout, abort, capacity, JSON, turn-shape, schema, unauthorized-address, call-count, or final-consistency failure preserves the upstream Candidate unchanged and marks the review degraded. An empty Witness lane must be the complete object `{"kind":"none","ranges":[],"attacked_premise":"","supporting_block_ids":[]}`. Missing fields, `null`, string `"null"`, empty objects, old primary/secondary slots, direction fields, and extra fields all fail closed; there is no coercion or partial-JSON repair. Witness raw trace contains the exact assistant JSON text, while normalized Witness trace contains the parsed object only after native `JSON.parse` succeeds. If a Finalizer encodes EOF as `exit_block_id_exclusive = terminalBlockId + 1`, the Harness mechanically normalizes that one-past-end address to `null`; it does not infer that a hard root is valid or that its Owner reaches EOF, and every other nonexistent or invalid exit still fails closed. For one submitted range, the Harness may mechanically discard ordinary `OUT` spill only when the remaining addresses are non-empty and form exactly one continuous island inside the declared run; all-`OUT`, cross-run, unavailable-block, or multi-island submissions still fail closed. There is no retry, fourth call, best-of-N, voting, block ledger, keyword scan, or code-side semantic repair.
...
`Content-Type: application/json` serializes the Doubao request body. Witness runs with `tools=[]`; the provider payload removes tools, tool choice, parallel tool calls, and reasoning effort, sets thinking disabled, and supplies the exact `PiNativeSemanticWitnessSchema` through `response_format: {"type":"json_schema","json_schema":{"strict":true,...}}`. Native Ark/Doubao support was verified with a real HTTP 200 smoke before enabling this route. Provider-side constrained decoding is still not semantic authority: the Harness accepts only one normal-stop assistant response containing exactly one pure JSON text block, parses it with native `JSON.parse`, then independently checks the same TypeBox schema plus lane cross-field rules. The one-call hard limit and 2400-token output cap remain unchanged.
